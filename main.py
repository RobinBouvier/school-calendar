...
def transformTp(ADE):
    ...
    """
    for ligne in range(len(ADE)): #on itère toute les lignes du dataframe
        #si la colonne TD contient un TP on le change par le TD auquel il appartient
...
def getTd(nomTd:str, ADE):
    """
    Le fonction getTd permet de récuperer les cours uniquement pour le TD concerné.
    ...
    -------
    ADE : pandas.Dataframe
        ADE contient le fichier csv avec les lignes qui en sont pas du bon TD qui ont été supprimé.
    ...
    for ligne in range(len(ADE)): #pour chaque ligne du dataframe
        ...
        if nomTd != ADE.loc[int(ligne), "TD"] and nomTd + "-PT" != ADE.loc[int(ligne), "TD"] and "Cours" != ADE.loc[int(ligne), "TD"] and "Examen" != ADE.loc[int(ligne), "TD"]:
